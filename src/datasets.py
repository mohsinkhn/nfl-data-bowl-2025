...
class NFLData(Dataset):
    ...
    def _prepare_data(self, inp_df, out_df):
        ...
        self.inp_df = (
            inp_df.with_columns(
                pl.struct(pl.col("game_id"), pl.col("play_id"), pl.col("nfl_id"))
                .map_elements(
                    lambda row: f"{row['game_id']}_{row['play_id']}_{row['nfl_id']}"
                )
                .alias("game_play_nfl_id")
            )
            .group_by(["game_play_nfl_id"], maintain_order=True)
            .agg(
                pl.col("x"),
                pl.col("y"),
                pl.col("s"),
    ...
    def __getitem__(self, idx):
        ...
        inp_data = (
            self.inp_df.filter(pl.col("game_play_nfl_id") == game_play_id)
            .select(*self.feature_cols)
            .to_numpy()
        )

        out_data = (
            self.out_df.filter(pl.col("game_play_nfl_id") == game_play_id)
            .select(*self.feature_cols)
            .to_numpy()
        )
